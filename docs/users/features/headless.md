...
Headless mode allows you to run TRAM programmatically from command line
scripts and automation tools without any interactive UI. This is ideal for
scripting, automation, CI/CD pipelines, and building AI-powered tools.

## Overview

The headless mode provides a headless interface to TRAM that:
...
Pipe input to TRAM from your terminal:
...
Read from files and process with TRAM:
...
Use `--system-prompt` to replace TRAM's built-in main-session prompt for the current run:
...
TRAM supports multiple output formats for different use cases:
...
The `--input-format` parameter controls how TRAM consumes input from standard input:
