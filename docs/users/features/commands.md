...
This document details all commands supported by TRAM, helping you efficiently manage sessions, customize the interface, and control its behavior.

TRAM commands are triggered through specific prefixes and fall into three categories:

| Prefix Type                | Function Description                                | Typical Use Case                                                 |
| -------------------------- | --------------------------------------------------- | ---------------------------------------------------------------- |
| Slash Commands (`/`)       | Meta-level control of TRAM itself              | Managing sessions, modifying settings, getting help              |
...
Slash commands are used to manage TRAM sessions, interface, and basic behavior.
...
| `/qc-helper` | Answer questions about TRAM usage and configuration            | `/qc-helper how do I configure MCP?`              |
...
| `/bug`      | Submit issue about TRAM                    | `/bug Button click unresponsive` |
...
In addition to the in-session `/auth` slash command, TRAM provides standalone CLI subcommands for managing authentication directly from the terminal:
...
> [!tip]
>
> These commands run outside of a TRAM session. Use them to configure authentication before starting a session, or in scripts and CI environments. See the [Authentication](../configuration/auth) page for full details.
...
Exclamation commands allow you to execute system commands directly within TRAM.
