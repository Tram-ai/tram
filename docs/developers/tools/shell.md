...
This document describes the `run_shell_command` tool for TRAM.
...
- `action` (string, optional): One of `start`, `list`, `output`, `input`, `stop`. Defaults to `start`.
- `command` (string, required when `action=start`): The exact shell command to execute.
- `description` (string, optional): A brief description of the command's purpose, which will be shown to the user.
- `directory` (string, optional): The directory (relative to the project root) in which to execute the command. If not provided, the command runs in the project root.
- `is_background` (boolean, optional): Whether to run the command in background. Set to true for long-running processes.
- `pid` (number, required for `action=output|input|stop`): PID of the managed background shell session.
- `input` (string, required for `action=input`): Input text sent to the background PTY session.
- `tail_lines` (number, optional): Number of recent lines returned by `action=output`.

## How to use `run_shell_command` with TRAM

When using `run_shell_command`, the command is executed as a subprocess. You can control whether commands run in background or foreground using the `is_background` parameter.

Background sessions started with `is_background=true` are managed by PID and can be controlled via:

- `action=list`: list running background shell sessions
- `action=output`: read output for a specific PID
- `action=input`: send interactive input to a specific PID
- `action=stop`: terminate a specific PID

### Background Session Management

Background session operations (`list` / `output` / `input` / `stop`) use `pid` as the primary key.
...
- `Background PID`: PID for a managed background session.
...
**Note:** If `action` is omitted, `run_shell_command` uses `action=start`.
...
List running background shell sessions:

```bash
run_shell_command(action="list")
```

Read output from a background session:

```bash
run_shell_command(action="output", pid=12345, tail_lines=200)
```

Send input to a background session:

```bash
run_shell_command(action="input", pid=12345, input="status\n")
```

Stop a background session:

```bash
run_shell_command(action="stop", pid=12345)
```

## Configuration

You can configure the behavior of the `run_shell_command` tool by modifying your `settings.json` file or by using the `/settings` command in the TRAM.
...
When an interactive command is running, you can send input to it from the TRAM. To focus on the interactive shell, press `ctrl+f`. The terminal output, including complex TUIs, will be rendered correctly.

## Important notes

- **Security:** Be cautious when executing commands, especially those constructed from user input, to prevent security vulnerabilities.
- **Error handling:** Check the `Stderr`, `Error`, and `Exit Code` fields to determine if a command executed successfully.
- **Background processes:** When `is_background=true`, the tool returns immediately and stores the session as a managed background task keyed by PID.
- **Background execution choices:** Use `action=list|output|input|stop` with `pid` to manage background sessions directly from tool calls.
