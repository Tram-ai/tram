# MCP servers with TRAM

This document provides a guide to configuring and using Model Context Protocol (MCP) servers with TRAM.
...
TRAM integrates with MCP servers through a sophisticated discovery and execution system built into the core package (`packages/core/src/tools/`):
...
TRAM uses the `mcpServers` configuration in your `settings.json` file to locate and connect to MCP servers. This configuration supports multiple servers with different transport mechanisms.
...
TRAM supports OAuth 2.0 authentication for remote MCP servers using SSE or HTTP transports. This enables secure access to MCP servers that require authentication.
...
When TRAM starts, it performs MCP server discovery through the following detailed process:
...
When TRAM receives this response, it will:
...
In addition to tools, MCP servers can expose predefined prompts that can be executed as slash commands within TRAM. This allows you to create shortcuts for common or complex queries that can be easily invoked by name.
...
```bash
/poem-writer --title="TRAM" --mood="reverent"
```

or, using positional arguments:

```bash
/poem-writer "TRAM" reverent
```
