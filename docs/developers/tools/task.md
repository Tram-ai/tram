...
This document describes the `task` tool for TRAM.
...
You can view available subagents by using the `/agents` command in TRAM.
...
Subagents are configured through TRAM's agent configuration system. Use the `/agents` command to:
