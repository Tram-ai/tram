# Installation Guide for TRAM with Source Tracking

This guide describes how to install Node.js and TRAM with source information tracking.

## Overview

The installation scripts automate the process of installing Node.js (if not present or below version 20) and TRAM, while capturing and storing the installation source information for analytics and tracking purposes.
...
- Installs TRAM globally with source information
- Stores the source information in `~/.tram/source.json`
...
1. The script accepts a `--source` parameter to specify where TRAM is being installed from
2. It installs Node.js if needed
3. It installs TRAM globally
4. It creates `~/.tram/source.json` with the specified source information
...
This is required to load the newly installed Node.js and TRAM into your PATH.
...
- Installs TRAM globally with source information
- Stores the source information in `%USERPROFILE%\.tram\source.json`

#### Prerequisites:

- **PowerShell (Administrator)**: The script must be run in PowerShell with Administrator privileges
- Internet connection for downloading Node.js and TRAM
...
1. The script accepts a `--source` or `-s` parameter to specify where TRAM is being installed from
...
4. It installs TRAM globally using npm
5. It creates `%USERPROFILE%\.tram\source.json` with the specified source information
...
This feature implements the ability to capture and store the installation source of the TRAM package. The source information is used for analytics and tracking purposes.

### Storage Location

The installation source is stored in a separate file at:

- **Unix/Linux/macOS**: `~/.tram/source.json`
- **Windows**: `%USERPROFILE%\.tram\source.json` (equivalent to `C:\Users\{username}\.tram\source.json`)
...
2. **Analytics**: Helps understand how users are discovering and installing TRAM
...
```bash
cat ~/.tram/source.json
```

**Windows:**

```cmd
type %USERPROFILE%\.tram\source.json
```
...
- Ensure NVM is installed: `curl -o- https://tram-assets.oss-cn-hangzhou.aliyuncs.com/installation/install_nvm.sh | bash`
...
- The scripts require internet access to download Node.js and TRAM
