...
# TRAM Installation Script
# This script installs Node.js (via NVM) and TRAM CLI
...
install_nvm() {
    ...
    if "${DOWNLOAD_CMD}" "${DOWNLOAD_ARGS}" "https://tram-assets.oss-cn-hangzhou.aliyuncs.com/installation/install_nvm.sh" > "${NVM_INSTALL_TEMP}"; then
    ...
    # Check if profile file is writable
        ...
        {
            echo ""
            echo "# NVM configuration (added by TRAM installer)"
...
fix_npm_permissions() {
    ...
    if [[ "${use_user_dir}" == true ]]; then
        ...
        if [[ -n "${PROFILE_FILE}" ]] && ! grep -q '.npm-global/bin' "${PROFILE_FILE}" 2>/dev/null; then
            {
                echo ""
                echo "# NPM global bin (added by TRAM installer)"
...
install_qwen_code() {
    ...
    if command_exists qwen; then
        ...
        log_success "TRAM is already installed: ${QWEN_VERSION}"
    ...
    # Install TRAM
    log_info "Installing TRAM..."
    if npm install -g @tram-ai/tram@latest --registry https://registry.npmmirror.com; then
        log_success "TRAM installed successfully!"

        # Verify installation
        if command_exists qwen; then
            local qwen_version
            qwen_version=$(qwen --version 2>/dev/null) || qwen_version="unknown"
            log_info "TRAM version: ${qwen_version}"
        fi
    else
        log_error "Failed to install TRAM!"
...
EOF
    ...
    log_success "Installation source saved to ~/.tram/source.json"
}
...
main() {
    ...
    if command_exists qwen; then
        log_success "TRAM is ready to use!"
        echo ""
        echo "You can now run: qwen"
        echo ""
        # Auto-start qwen
        log_info "Starting TRAM..."
        echo ""
        exec qwen
    else
        log_warning "TRAM command not found in current session"
        echo ""
        echo "To use TRAM immediately without restarting your terminal,"
