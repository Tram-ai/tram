...
describe("useGeminiStream", () => {
  ...
  describe("Tool-use summary generation", () => {
    ...
    it("fires generation with tool input/output when enabled", async () => {
      ...
      expect(callArgs[3]).toBe("tram-fast");
    ...
    it("should render descriptions from subject-bearing thought chunks", async () => {
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield {
            ...
            value: {
              subject: "",
              description: " user mentioned globally installed tram,",
            },
          };
          yield {
            type: ServerGeminiEventType.Finished,
      ...
      await waitFor(() => {
        expect(mockAddItem).toHaveBeenCalledWith(
          expect.objectContaining({
            type: "gemini_thought",
            text: "The user mentioned globally installed tram,",
          }),
          expect.any(Number),
        );
      });
      expect(result.current.thought).toEqual({
        subject: "Evaluating installation approach",
        description: "The user mentioned globally installed tram,",
      });
    });
