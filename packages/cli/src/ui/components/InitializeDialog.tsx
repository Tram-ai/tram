...
export function InitializeDialog({
  settings,
  onClose,
}: InitializeDialogProps): React.JSX.Element {
  ...
  const applyConfig = useCallback(
    (payload: WebSettingsPayload) => {
      ...
      try {
        ...
        const modelProviders = reloaded.merged.modelProviders as
          | ModelProvidersConfig
          | undefined;
        if (modelProviders) {
          config.reloadModelProvidersConfig(modelProviders);
        }
        const authType = reloaded.merged.security?.auth?.selectedType as
          | AuthType
          | undefined;
        if (authType) {
