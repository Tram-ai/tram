...
describe("ThemeManager", () => {
  ...
  describe("auto theme detection", () => {
    it("should select Tram Dark when terminal is detected as dark", () => {
      ...
      expect(themeManager.getActiveTheme().name).toBe("Tram Dark");
    });

    it("should select Tram Light when terminal is detected as light", () => {
      vi.mocked(detectModule.detectTerminalTheme).mockReturnValue("light");
      const result = themeManager.setActiveTheme(AUTO_THEME_NAME);
      expect(result).toBe(true);
      expect(themeManager.getActiveTheme().name).toBe("Tram Light");
    });
    ...
    it("should resolve async auto theme with Tram Light for light", async () => {
    ...
    it("should resolve async auto theme with Tram Dark for dark", async () => {
      vi.mocked(detectModule.detectTerminalThemeAsync).mockResolvedValue(
        "dark",
      );
      await themeManager.resolveAutoThemeAsync();
