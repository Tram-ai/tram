...
export type AuthUiState = {
  ...
  tramAuthState: ReturnType<typeof useTramAuth>['tramAuthState'];
};
...
export const useAuthCommand = (
  ...
  // -- Legacy auth select (TRAM OAuth / direct) ----------------------------
  ...
  const handleAuthSelect = useCallback(
    async (authType: AuthType | undefined, credentials?: OpenAICredentials) => {
      ...
      // TRAM OAuth or other direct auth
      try {
  ...
  // -- Validate TRAM_DEFAULT_AUTH_TYPE env var on mount --------------------

  useEffect(() => {
    const val = process.env['TRAM_DEFAULT_AUTH_TYPE'];
    const valid = [
      AuthType.TRAM_OAUTH,
      AuthType.USE_OPENAI,
