...
export const SETTINGS_DIRECTORY_NAME = TRAM_DIR;

// Lazy getters: must NOT be top-level consts. `TRAM_HOME` may be resolved
// from `~/.env` or `~/.tram/.env` by `preResolveHomeEnvOverrides()` in
...
// Backwards-compatible eager exports for callers that still import these.
// New code should prefer `getUserSettingsPath()` / `getUserSettingsDir()`.
...
function getUserLevelEnvPaths(): Set<string> {
  ...
  const legacyQwenEnv = path.normalize(path.join(homeDir, TRAM_DIR, '.env'));
...
function findEnvFile(
  ...
  const globalQwenDir = Storage.getGlobalTramDir();
  const legacyQwenDir = path.normalize(path.join(homeDir, TRAM_DIR));
  ...
  // Home-dir candidates in priority order: globalTramDir/.env, then legacy
  // ~/.tram/.env (only when TRAM_HOME redirects), then ~/.env.
  // Users who add `TRAM_HOME=` to an existing global env file shouldn't lose
  ...
  while (true) {
    if (currentDir === homeDir) {
      ...
      // Workspace step: prefer .tram/.env, then plain .env.
...
export function loadEnvironment(settings: Settings): void {
  ...
  if (envFilePath) {
    try {
      ...
      // homeScoped: `.env` lives under the user's home Tram dir or `~/.env` —
      //   only these may set TRAM_HOME / TRAM_RUNTIME_DIR.
      // tramScoped: any `.env` whose immediate parent is `.tram` (including
      //   `<repo>/.tram/.env`) — exempt from the user `excludedEnvVars` list.
      const isHomeScopedEnvFile = userLevelPaths.has(normalizedEnvFilePath);
      const isQwenScopedEnvFile =
        isHomeScopedEnvFile ||
        path.basename(path.dirname(normalizedEnvFilePath)) === TRAM_DIR;
