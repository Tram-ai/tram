...
import { QWEN_DIR } from "@tram-ai/tram-core";
...
// must keep going through `getUserSettingsPath()` to pick up `TRAM_HOME`
...
describe("Settings Loading and Merging", () => {
  ...
  describe("loadSettings", () => {
    ...
    it("should add version field to V2 settings without version and write to disk", () => {
      ...
      expect(writtenContent.model?.name).toBe("tramr");
    ...
    it("should correctly handle partially migrated settings without version field", () => {
      ...
      expect(writtenContent.model).toEqual({ name: "tramr" });
    ...
    describe("when TRAM_CODE_SYSTEM_SETTINGS_PATH is set", () => {
      const MOCK_ENV_SYSTEM_SETTINGS_PATH = "/mock/env/system/settings.json";

      beforeEach(() => {
        process.env["TRAM_CODE_SYSTEM_SETTINGS_PATH"] =
          MOCK_ENV_SYSTEM_SETTINGS_PATH;
      });

      afterEach(() => {
        delete process.env["TRAM_CODE_SYSTEM_SETTINGS_PATH"];
      });
  ...
  describe("loadEnvironment", () => {
    function setup({
      ...
      const geminiEnvPath = path.resolve(path.join(TRAM_DIR, ".env"));
    ...
    describe("settings.env field", () => {
      ...
      it("should never set TRAM_HOME or TRAM_RUNTIME_DIR from settings.env", () => {
        ...
        delete process.env["TRAM_HOME"];
        delete process.env["TRAM_RUNTIME_DIR"];

        const userSettingsContent: Settings = {
          env: {
            TRAM_HOME: "/redirected/by/settings",
            TRAM_RUNTIME_DIR: "/redirected/runtime",
            HARMLESS_VAR: "ok",
          },
        };
        ...
        expect(process.env["TRAM_HOME"]).toBeUndefined();
        expect(process.env["TRAM_RUNTIME_DIR"]).toBeUndefined();
