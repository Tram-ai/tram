...
import type { Config, SandboxConfig } from "@tram-ai/tram-core";
import {
  FatalSandboxError,
  Storage,
  isSubpath,
...
const LOCAL_DEV_SANDBOX_IMAGE_NAME = "tram-sandbox";
const SANDBOX_NETWORK_NAME = "tram-sandbox";
const SANDBOX_PROXY_NAME = "tram-sandbox-proxy";
...
function entrypoint(workdir: string, cliArgs: string[]): string[] {
  ...
  const cliCmd =
    process.env["NODE_ENV"] === "development"
      ...
      : process.env["DEBUG"]
        ? `node --inspect-brk=0.0.0.0:${process.env["DEBUG_PORT"] || "9229"} $(which tram)`
        : "tram";
...
export async function start_sandbox(
  ...
  if (config.command === "sandbox-exec") {
    ...
    // start and set up proxy if TRAM_SANDBOX_PROXY_COMMAND is set
    const proxyCommand = process.env["TRAM_SANDBOX_PROXY_COMMAND"];
  ...
  // determine full path for tram to distinguish linked vs installed setting
  ...
  // if BUILD_SANDBOX is set, then call scripts/build_sandbox.js under tram repo
  //
  // note this can only be done with binary linked from tram repo
  if (process.env["BUILD_SANDBOX"]) {
    if (!gcPath.includes("tram/packages/")) {
      throw new FatalSandboxError(
        "Cannot build sandbox using installed TRAM binary; " +
          "run `npm link ./packages/cli` under TramCode-cli repo to switch to linked binary.",
      );
    } else {
      ...
      execSync(
        `cd ${gcRoot} && node scripts/build_sandbox.js -s ${buildArgs}`,
        {
          ...
          env: {
            ...process.env,
            TRAM_SANDBOX: config.command, // in case sandbox is enabled via flags (see config.ts under cli package)
          },
        },
      );
    }
  ...
  if (!(await ensureSandboxImageIsPresent(config.command, image))) {
    const remedy =
      image === LOCAL_DEV_SANDBOX_IMAGE_NAME
        ? "Try running `npm run build:all` or `npm run build:sandbox` under the tram repo to build it locally, or check the image name and your network connection."
        : "Please check the image name, your network connection, or notify tram-dev@service.alibaba.com if the issue persists.";
    throw new FatalSandboxError(
  ...
  // Mount user settings at /home/node/.tram and at the canonical host path
  // used by TRAM_HOME, unless that host path is already covered by a broader
  ...
  // Pass TRAM_HOME so the sandboxed CLI resolves the global tram dir to the
  // same path the host did, instead of relying on the /home/node/.tram mount
  // being the default fallback.
  args.push("--env", `TRAM_HOME=${userSettingsDirContainerPath}`);

  // Mount the runtime base dir and pass TRAM_RUNTIME_DIR when it diverges
  // from the global tram dir; otherwise the existing user-settings mount
  // already covers it.
  ...
  if (!runtimeSameAsUserSettings) {
    args.push("--env", `TRAM_RUNTIME_DIR=${runtimeBaseDirContainerPath}`);
  }
  ...
  // TRAM_SANDBOX_PROXY_COMMAND implies HTTPS_PROXY unless HTTP_PROXY is set
  const proxyCommand = process.env["TRAM_SANDBOX_PROXY_COMMAND"];
  ...
  const isIntegrationTest =
    process.env["TRAM_CODE_INTEGRATION_TEST"] === "true";
  let containerName;
  if (isIntegrationTest) {
    containerName = `tram-integration-test-${randomBytes(4).toString("hex")}`;
  ...
  // copy TRAM_CODE_TEST_VAR for integration tests
  if (process.env["TRAM_CODE_TEST_VAR"]) {
    args.push(
      "--env",
      `TRAM_CODE_TEST_VAR=${process.env["TRAM_CODE_TEST_VAR"]}`,
    );
  }
  ...
  // copy OPENAI_API_KEY and related env vars for TRAM
  ...
  // tests that need to access host's ~/.tram (e.g., --resume functionality)
  ...
  if (useCurrentUser) {
    ...
    // allowing tests that need to access host's ~/.tram (e.g., --resume) to work.
    ...
    // finally switch to that user to run the tram process. This is
