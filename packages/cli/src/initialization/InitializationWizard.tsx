...
} from "../utils/modelProviderIds.js";
...
type EditableProviderField = "baseUrl" | "apiKey" | "localModelsInput";
...
function appendProviderModels(
  ...
  for (const upstreamModelId of config.modelIds) {
    const preferredId = upstreamModelId;
    const localModelId = usedIds.has(preferredId)
      ? getNextAvailableModelId(
          authType === AuthType.USE_OPENAI ? upstreamModelId : preferredId,
          usedIds,
        )
      : preferredId;
    usedIds.add(localModelId);
...
function WizardApp({
  ...
  const [pollinationsAuthStatus, setPollinationsAuthStatus] = useState<
  ...
  const [editingField, setEditingField] =
    useState<EditableProviderField | null>(null);
  ...
  const clampedStart = Math.max(
    ...
    Math.min(
      Math.floor(
        (modelOptions.length + 1 - PAGE_SIZE + columns - 1) / columns,
      ) * columns,
      visibleModelStart,
    ),
  );

  const visibleModelEnd = Math.min(
  ...
  const selectedProviders = useMemo(
    () =>
      PROVIDER_PRESETS.filter((provider) =>
  ...
  useEffect(() => {
    ...
    const loadModels = async () => {
      ...
      if (enableLocalModelList) {
        const localModels = parseLocalModelsInput(
          currentConfig.localModelsInput,
        );
      ...
      if (!disposed) {
        ...
        const initial = models.findIndex((m) =>
          currentConfig.modelIds.includes(m),
        );
  ...
  const queueSelectedProvidersForConfig = () => {
    const existingProviderIds = new Set(
      providerConfigs.map((cfg) => cfg.providerId),
    );
    ...
    const hasPollinations = configs.some(
      (c) => c.providerId === "pollinations",
    );
    if (hasPollinations) {
      setStep("pollinationsAuth");
      startPollinationsAuth(
        startIndex + configs.findIndex((c) => c.providerId === "pollinations"),
      );
      return;
    }
  ...
  const startPollinationsAuth = (configIdx: number) => {
    ...
    const app = express();
    const relayUrl =
      "https://enter.pollinations.ai/authorize?app_key=pk_Bl8CdIWBVPsm4IPa&redirect_url=http%3A%2F%2Flocalhost%3A9991%2Fcallback&expiry=360";
  ...
  const updateCurrentConfig = (
    ...
    setProviderConfigs((prev) =>
      prev.map((config, idx) =>
        idx === configIndex ? updater(config) : config,
      ),
    );
  };
  ...
  const saveSettings = () => {
    ...
    settings.setValue(
      scope,
      "modelProviders",
      normalizeModelProvidersConfig(nextProviders),
    );
    settings.setValue(scope, "env", nextEnv);
    settings.setValue(
      scope,
      "security.auth.selectedType",
      defaultProvider.authType,
    );
    ...
    if (themeOptions.length > 0) {
      settings.setValue(
        scope,
        "ui.theme",
        themeOptions[themeCursor] || themeOptions[0],
      );
    }
  ...
  useInput((input, key) => {
    if (key.escape) {
      ...
      if (step === "theme") {
        setStep(
          proxyModes[proxyModeCursor] === "custom"
            ? "proxyCustom"
            : "proxyMode",
        );
        return;
    ...
    if (step === "providers") {
      if (key.upArrow || key.leftArrow) {
        setProviderCursor((cursor) =>
          moveCursor(cursor, -1, PROVIDER_PRESETS.length),
        );
        return;
      }
    ...
    if (step === "approval") {
      if (key.upArrow || key.leftArrow) {
        setApprovalCursor((cursor) =>
          moveCursor(cursor, -1, APPROVAL_OPTIONS.length),
        );
        return;
      }
      if (key.downArrow || key.rightArrow) {
    ...
    if (step === "proxyMode") {
      if (key.upArrow || key.leftArrow) {
        setProxyModeCursor((cursor) =>
          moveCursor(cursor, -1, proxyModes.length),
        );
        return;
      }
      if (key.downArrow || key.rightArrow) {
  ...
  const renderProviderConfigStep = () => {
    ...
    return (
      <Box flexDirection="column" gap={1}>
        ...
        <Text>
          {t("API key env var") + ": " + (currentConfig.envKey || "-")}
        </Text>
        {fields.map((field, index) => {
          const isEditingThisRow = editingField === field.key;
          const editingDisplay =
            field.key === "apiKey"
              ? "*".repeat(editingValue.length)
              : editingValue;
          const cursor = isEditingThisRow ? "|" : "";
          return (
            <Text
              key={field.label}
              color={index === providerFieldCursor ? "cyan" : undefined}
            >
      ...
      {isModelLoading ? (
        ...
        <>
          <Box flexDirection="row" flexWrap="wrap">
            {modelOptions
              .slice(clampedStart, visibleModelEnd)
              .map((model, index) => {
                ...
                return (
                  <Box key={model} width={COLUMN_WIDTH}>
                    <Text
                      color={actualIndex === modelCursor ? "cyan" : undefined}
                    >
                      {actualIndex === modelCursor ? "❯" : " "}{" "}
                      {selectedModelIds.has(model) ? "[x]" : "[ ]"} {model}
                    </Text>
                  </Box>
                );
              })}
            {clampedStart + PAGE_SIZE > modelOptions.length && (
              <Box width={COLUMN_WIDTH}>
                <Text
                  color={
                    modelCursor === modelOptions.length ? "cyan" : undefined
                  }
                >
                  {modelCursor === modelOptions.length ? "❯" : " "}{" "}
                  {t("Custom model...")}
                </Text>
              </Box>
            )}
          ...
          <Text color="gray">
            {t("Showing {{start}}-{{end}} of {{total}}", {
              start: String(
                Math.min(modelOptions.length + 1, clampedStart + 1),
              ),
              end: String(
                Math.min(modelOptions.length + 1, clampedStart + PAGE_SIZE),
              ),
  ...
  const renderCustomModelInputStep = () => (
    <Box flexDirection="column" gap={1}>
      ...
      <Text color="gray">
        {t("Press Enter to confirm and keep selecting models.")}
      </Text>
  ...
  const renderProviderContinueStep = () => {
    const options = [t("Yes, add another provider"), t("No, continue setup")];
    ...
    return (
      <Box flexDirection="column" gap={1}>
        ...
        {options.map((option, index) => (
          <Text
            key={option}
            color={index === providerContinueCursor ? "cyan" : undefined}
          >
    ...
    return (
      <Box flexDirection="column" gap={1}>
        ...
        {labels.map((label, index) => (
          <Text
            key={label}
            color={index === proxyModeCursor ? "cyan" : undefined}
          >
  ...
  const renderThemeStep = () => {
    const previewTheme =
      themeOptions.length > 0
        ? themeManager.getTheme(themeOptions[themeCursor])
        : undefined;
    ...
    return (
      <Box flexDirection="column" gap={1}>
        ...
        <Text color="gray">
          {t("This list follows built-in /theme options.")}
        </Text>
        <Box flexDirection={useTwoColumnLayout ? "row" : "column"} gap={2}>
          <Box
            flexDirection="column"
            width={useTwoColumnLayout ? 44 : undefined}
          >
            {themeOptions.map((name, index) => (
              <Text
                key={name}
                color={index === themeCursor ? "cyan" : undefined}
              >
          ...
          {previewTheme && (
            <Box flexDirection="column" marginTop={useTwoColumnLayout ? 0 : 1}>
              ...
              <Text color={previewTheme.colors.AccentBlue}>
                {t("Preview: info text")}
              </Text>
              <Text color={previewTheme.colors.AccentGreen}>
                {t("Preview: success text")}
              </Text>
              <Text color={previewTheme.colors.AccentRed}>
                {t("Preview: error text")}
              </Text>
            </Box>
          )}
        </Box>
    ...
    return (
      <Box flexDirection="column" gap={1}>
        ...
        <Text>
          {t("Providers") +
            ": " +
            providerConfigs.map((p) => t(p.providerLabelKey)).join(", ")}
        </Text>
        <Text>
          {t("Default provider") +
            ": " +
            (providerConfigs[0] ? t(providerConfigs[0].providerLabelKey) : "-")}
        </Text>
        <Text>
          {t("Default model") + ": " + (providerConfigs[0]?.modelIds[0] || "-")}
        </Text>
        <Text>
          {t("Selected models") +
            ": " +
            providerConfigs
              .map((p) => `${t(p.providerLabelKey)}(${p.modelIds.join(", ")})`)
              .join("; ")}
        </Text>
        <Text>
          {t("Approval mode") + ": " + APPROVAL_OPTIONS[approvalCursor].title}
        </Text>
    ...
    return (
      <Box flexDirection="column" gap={1}>
        ...
        {pollinationsAuthStatus === "success" && (
          <Text color="green">
            {t("Authorization successful! Returning to wizard...")}
          </Text>
        )}
    ...
    >
      {step !== "done" && (
        <>
          ...
          <Text color="gray">
            {t("Esc to go back, Esc again at provider list to cancel.")}
          </Text>
        </>
      )}
