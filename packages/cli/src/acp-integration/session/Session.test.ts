  ...
  describe('prompt', () => {
    describe('auto-compress', () => {
      ...
      it('stops cron-fired ACP prompt before sending when the session token limit is exceeded', async () => {
        ...
        // TODO(merge-v0.15.10): test ordering scrambled by previous merge - "ours" had assertions for available_commands_update but block is in middle of cron test
        expect(mockGeminiClient.tryCompressChat).toHaveBeenCalledTimes(2);
        expect(tokenLimitDiagnosticCount()).toBe(diagnosticCountBefore);
      });

      // TODO(merge-v0.15.10): scrambled merge - "ours" had 'swallows errors' test for sendAvailableCommandsUpdate, "theirs" added 'does not auto-compress slash commands' test. Taking theirs since 'swallows errors' was already added earlier in file.
