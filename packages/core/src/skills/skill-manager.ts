...
export class SkillManager {
  ...
  getSkillsBaseDirs(level: SkillLevel): string[] {
    switch (level) {
      ...
      case "user":
        return SKILL_PROVIDER_CONFIG_DIRS.map((v) =>
          v === TRAM_DIR
            ? path.join(Storage.getGlobalTramDir(), SKILLS_CONFIG_DIR)
  ...
  private async listSkillsAtLevel(level: SkillLevel): Promise<SkillConfig[]> {
    ...
    // so the order defines implicit precedence (.tram > .agent > .cursor > ...).
  ...
  private async ensureUserSkillsDir(): Promise<void> {
    const baseDir = path.join(Storage.getGlobalTramDir(), SKILLS_CONFIG_DIR);
    try {
