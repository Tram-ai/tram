...
/**
 * Converter for Gemini extensions to TRAM format.
 */
...
/**
 * Converts a Gemini extension config to TRAM format.
 * @param extensionDir Path to the Gemini extension directory
 * @returns TRAM ExtensionConfig
 */
export function convertGeminiToTramConfig(
  extensionDir: string,
): ExtensionConfig {
...
/**
 * Converts a complete Gemini extension package to TRAM format.
...
export async function convertGeminiExtensionPackage(
  ...
  const geminiConfig = convertGeminiToTramConfig(extensionDir);
  ...
  try {
    ...
    // Step 3: Create tram-extension.json with converted config
...
export function isGeminiExtensionConfig(extensionDir: string) {
  ...
  // If it has Gemini-specific fields but not TRAM-specific fields, likely Gemini
  return true;
}
