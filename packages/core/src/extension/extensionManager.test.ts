...
import { TRAM_DIR } from "../config/storage.js";
import {
  ExtensionManager,
  SettingScope,
...
const EXTENSIONS_DIRECTORY_NAME = path.join(TRAM_DIR, "extensions");
...
function createExtension({
  ...
  if (addContextFile) {
    fs.writeFileSync(path.join(extDir, "TRAM.md"), "context");
  }
...
describe("extension tests", () => {
  ...
  beforeEach(() => {
    tempHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "tram-test-home-"));
    tempWorkspaceDir = fs.mkdtempSync(
      path.join(tempHomeDir, "tram-test-workspace-"),
    );
  ...
  describe("loadExtension", () => {
    ...
    it("should load context file path when TRAM.md is present", async () => {
      createExtension({
        extensionsDir: userExtensionsDir,
        name: "ext1",
        version: "1.0.0",
      ...
      expect(ext1?.contextFiles).toEqual([
        path.join(userExtensionsDir, "ext1", "TRAM.md"),
      ]);
    ...
    it("should use default TRAM.md when contextFileName is empty array", async () => {
      ...
      fs.writeFileSync(path.join(extDir, "TRAM.md"), "context content");
      ...
      expect(ext?.contextFiles).toEqual([
        path.join(userExtensionsDir, "ext-empty-context", "TRAM.md"),
      ]);
    });
