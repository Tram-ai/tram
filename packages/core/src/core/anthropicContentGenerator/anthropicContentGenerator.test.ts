...
describe("AnthropicContentGenerator", () => {
  ...
  beforeEach(async () => {
    ...
    mockConfig = {
      ...
      resolveUpstreamModelId: vi.fn().mockImplementation((id: string) => id),
  ...
  it("passes a TramCode User-Agent header to the Anthropic SDK", async () => {
    ...
    expect(headers["User-Agent"]).toContain("TramCode/1.2.3");
  ...
  it("merges customHeaders into defaultHeaders (does not replace defaults)", async () => {
    ...
    expect(headers["User-Agent"]).toContain("Tram/1.2.3");
  ...
  describe("per-request anthropic-beta header", () => {
    ...
    it("keeps customHeaders + User-Agent in defaultHeaders while sending computed anthropic-beta per-request", async () => {
      ...
      expect(defaultHeaders["User-Agent"]).toContain("Tram/1.2.3");
  ...
  // https://github.com/tram-ai/tram/issues/3786 — DeepSeek's
