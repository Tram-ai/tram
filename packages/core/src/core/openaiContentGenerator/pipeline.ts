...
import {
  ...
  type OpenAIContentConverterClass,
} from "./converter.js";
...
export class ContentGenerationPipeline {
  ...
  private converter: OpenAIContentConverterClass;
  ...
  private createRequestContext(
    ...
    // TODO(merge-v0.15.10): RequestContext shape diverged — ours has authType (consumed by error logging), upstream's types.ts shape has modalities (consumed by converters). The current local RequestContext (errorHandler.ts) lacks modalities; revisit once errorHandler/types unification is decided.
    return {
