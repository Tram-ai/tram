...
export function convertOpenAIChunkToGemini(
  ...
  if (choice) {
    ...
    if (choice.finish_reason) {
      ...
      // Some providers (e.g. DashScope/TRAM) send "stop" or "tool_calls"
...
/**
 * Backward-compat class wrapper around the standalone converter functions.
 *
 * Upstream qwen-code v0.15.10 refactored the OpenAIContentConverter class into
 * standalone exported functions plus a const aggregate. To keep TRAM's existing
 * class-form call sites working (`new OpenAIContentConverter(model, schemaCompliance, modalities)`
 * with `setModel(...)`, `setModalities(...)`, `resetStreamingToolCalls()`,
 * and instance method calls like `converter.convertOpenAIChunkToGemini(chunk)`),
 * we expose this thin wrapper. Each instance method synthesizes a RequestContext
 * from the instance state and delegates to the standalone function.
 *
 * For new code, prefer calling the standalone functions directly (or the static
 * const aggregate of the same name below) with an explicit RequestContext built
 * from the pipeline.
 */
export class OpenAIContentConverterClass {
  ...
  setSchemaCompliance(schemaCompliance: SchemaComplianceMode): void {
    this.schemaCompliance = schemaCompliance;
  }
  ...
  private buildContext(overrides: Partial<RequestContext> = {}): RequestContext {
    return {
      model: this.model,
      modalities: this.modalities,
      startTime: Date.now(),
      toolCallParser: this.streamingToolCallParser,
      ...overrides,
    };
  }
  ...
  convertGeminiToolParametersToOpenAI(
    ...
    return convertGeminiToolParametersToOpenAI(parameters);
  }

  async convertGeminiToolsToOpenAI(
    geminiTools: ToolListUnion,
  ): Promise<OpenAI.Chat.ChatCompletionTool[]> {
    return convertGeminiToolsToOpenAI(geminiTools, this.schemaCompliance);
  }
  ...
  convertGeminiRequestToOpenAI(
    ...
    return convertGeminiRequestToOpenAI(request, this.buildContext(), options);
  }

  convertGeminiResponseToOpenAI(
    response: GenerateContentResponse,
  ): OpenAI.Chat.ChatCompletion {
    return convertGeminiResponseToOpenAI(response, this.buildContext());
  }

  convertOpenAIResponseToGemini(
    openaiResponse: OpenAI.Chat.ChatCompletion,
  ): GenerateContentResponse {
    return convertOpenAIResponseToGemini(openaiResponse, this.buildContext());
  }
  ...
  convertOpenAIChunkToGemini(
    ...
    return convertOpenAIChunkToGemini(
      chunk,
      contextOverride ?? this.buildContext(),
    );
  }
}

/**
 * Backward-compat aggregate exposing the standalone converter functions
 * via the historical `OpenAIContentConverter` namespace. Used by pipeline.ts
 * and other call sites that invoke the converter via static-style member
 * access (e.g. `OpenAIContentConverter.convertGeminiRequestToOpenAI(...)`).
 *
 * Also callable as a constructor for the legacy class-form API.
 */
export const OpenAIContentConverter: {
  ...
  new (
    ...
    modalities?: InputModalities,
  ): OpenAIContentConverterClass;
} = Object.assign(
  OpenAIContentConverterClass as unknown as new (
    model: string,
    schemaCompliance?: SchemaComplianceMode,
    modalities?: InputModalities,
  ) => OpenAIContentConverterClass,
