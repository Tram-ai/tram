...
export class Logger {
  private tramDir: string | undefined;
  ...
  async initialize(): Promise<void> {
    ...
    this.tramDir = this.storage.getProjectTempDir();
    this.logFilePath = path.join(this.tramDir, LOG_FILE_NAME);

    try {
      await fs.mkdir(this.tramDir, { recursive: true });
  ...
  private _checkpointPath(tag: string): string {
    ...
    return path.join(this.tramDir, `checkpoint-${encodedTag}.json`);
  }
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.tramDir!, `checkpoint-${tag}.json`);
    try {
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.tramDir) {
      this.debugLogger.error(
