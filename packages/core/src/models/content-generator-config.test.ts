...
describe("buildAgentContentGeneratorConfig", () => {
  ...
  describe("with registry-resolved model", () => {
    ...
    it("should set requestModel to upstreamModelId when present", () => {
      const resolvedModelWithUpstream: ResolvedModelConfig = {
        ...resolvedModel,
        id: "local-conflict-safe-id",
        upstreamModelId: "claude-3-5-sonnet-20241022",
      };
      const config = createMockConfig(parentConfig, resolvedModelWithUpstream);

      const result = buildAgentContentGeneratorConfig(
        config,
        "local-conflict-safe-id",
        { authType: "anthropic" },
      );

      expect(result.model).toBe("local-conflict-safe-id");
      expect(result.requestModel).toBe("claude-3-5-sonnet-20241022");
    });

    it("should fall back requestModel to id when upstreamModelId is absent", () => {
      const config = createMockConfig(parentConfig, resolvedModel);
      ...
      expect(result.model).toBe("registry-model-id");
      expect(result.requestModel).toBe("registry-model-id");
    });
  });
