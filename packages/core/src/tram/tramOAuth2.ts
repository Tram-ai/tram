...
/**
 * TRAM OAuth2 credentials interface
 */
export interface TramCredentials {
  access_token?: string;
...
/**
 * TRAM OAuth2 client interface
 */
export interface ITramOAuth2Client {
  setCredentials(credentials: TramCredentials): void;
  getCredentials(): TramCredentials;
...
/**
 * TRAM OAuth2 client implementation
 */
export class TramOAuth2Client implements ITramOAuth2Client {
  private credentials: TramCredentials = {};
  ...
  async pollDeviceToken(options: {
    ...
    const bodyData = {
      grant_type: TRAM_OAUTH_GRANT_TYPE,
      client_id: TRAM_OAUTH_CLIENT_ID,
...
/**
 * Global event emitter instance for TramOAuth2 authentication events
 */
export const tramOAuth2Events = new EventEmitter();

export async function getTramOAuthClient(
  config: Config,
  options?: { requireCachedCredentials?: boolean },
): Promise<TramOAuth2Client> {
  ...
  try {
    ...
    const result = await authWithTramDeviceFlow(client, config);
    if (!result.success) {
...
async function authWithTramDeviceFlow(
  ...
  tramOAuth2Events.once(TramOAuth2Event.AuthCancel, cancelHandler);
  ...
  try {
    ...
    tramOAuth2Events.emit(TramOAuth2Event.AuthUri, deviceAuth);
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      try {
        ...
        if (isDeviceTokenSuccess(tokenResponse)) {
          ...
          // Convert to TramCredentials format
          ...
          await cacheTramCredentials(credentials);

          // IMPORTANT:
          // SharedTokenManager maintains an in-memory cache and throttles file checks.
          // If we only write the creds file here, a subsequent `getTramOAuthClient()`
...
async function cacheTramCredentials(credentials: TramCredentials) {
...
/**
 * Clear cached TRAM credentials from disk
 * This is useful when credentials have expired or need to be reset
 */
export async function clearTramCredentials(): Promise<void> {
  try {
    const filePath = getTramCachedCredentialPath();
