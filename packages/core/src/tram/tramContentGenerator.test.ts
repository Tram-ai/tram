...
vi.mock("./sharedTokenManager.js", () => ({
  SharedTokenManager: class {
    ...
    async getValidCredentials(
      ...
      try {
        const { token } = await tramClient.getAccessToken();
        if (token) {
          const credentials = tramClient.getCredentials();
          return credentials;
        }
      } catch (error) {
    ...
    setMockCredentials(credentials: TramCredentials | null): void {
...
describe("TramContentGenerator", () => {
  ...
  beforeEach(() => {
    ...
    tramContentGenerator = new TramContentGenerator(
      mockTramClient,
  ...
  describe("Error Handling and Retry Logic", () => {
    it("should retry once on authentication errors", async () => {
      ...
      vi.mocked(mockTramClient.getAccessToken).mockImplementation(async () => {
  ...
  describe("Token State Management", () => {
    ...
    it("should handle concurrent token refresh requests", async () => {
      ...
      vi.mocked(mockTramClient.refreshAccessToken).mockImplementation(
        async () => {
          refreshCallCount++;
