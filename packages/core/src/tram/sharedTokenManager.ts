...
interface MemoryCache {
  credentials: TramCredentials | null;
  fileModTime: number;
  lastCheck: number;
}

/**
 * Validates that the given data is a valid TramCredentials object
 *
 * @param data - The data to validate
 * @returns The validated credentials object
 * @throws Error if the data is invalid
 */
function validateCredentials(data: unknown): TramCredentials {
  if (!data || typeof data !== 'object') {
    throw new Error('Invalid credentials format');
  }

  const creds = data as Partial<TramCredentials>;
  ...
  return creds as TramCredentials;
}
...
export class SharedTokenManager {
   ...
   */
  private refreshPromise: Promise<TramCredentials> | null = null;
   ...
   *
   * @param tramClient - The OAuth2 client instance
  ...
  async getValidCredentials(
    tramClient: ITramOAuth2Client,
    forceRefresh = false,
  ): Promise<TramCredentials> {
    try {
      // Check if credentials file has been updated by other sessions
      await this.checkAndReloadIfNeeded(tramClient);
  ...
  private async checkAndReloadIfNeeded(
    ...
    this.checkPromise = this.performFileCheck(tramClient, now);
  ...
  private async performFileCheck(
    tramClient: ITramOAuth2Client | undefined,
    checkTime: number,
  ): Promise<void> {
    ...
    try {
      ...
      if (fileModTime > this.memoryCache.fileModTime) {
        await this.reloadCredentialsFromFile(tramClient);
   ...
   */
  private async forceFileCheck(tramClient?: ITramOAuth2Client): Promise<void> {
    try {
      const filePath = this.getCredentialFilePath();
  ...
  /**
   * Load credentials from the file system into memory cache and sync with tramClient
   */
  private async reloadCredentialsFromFile(
    ...
    try {
      ...
      // Sync with tramClient atomically - rollback on failure
      try {
        if (tramClient) {
          tramClient.setCredentials(credentials);
        }
      } catch (clientError) {
  ...
  private async performTokenRefresh(
    ...
    try {
      ...
      const currentCredentials = tramClient.getCredentials();
      ...
      await this.forceFileCheck(tramClient);
      ...
      if (
        ...
        // No need to call tramClient.setCredentials here as checkAndReloadIfNeeded already did it
        return this.memoryCache.credentials;
      }

      // Perform the actual token refresh
      const response = await tramClient.refreshAccessToken();
      ...
      const credentials: TramCredentials = {
   ...
   */
  private isTokenValid(credentials: TramCredentials): boolean {
   ...
   */
  getCurrentCredentials(): TramCredentials | null {
    return this.memoryCache.credentials;
  }
