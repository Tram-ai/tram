...
import { TRAM_DIR, unescapePath } from './paths.js';
...
`;
  ...
  describe('loadRules', () => {
    ...
    it('loads baseline rules into content', async () => {
      const rulesDir = path.join(projectRoot, TRAM_DIR, 'rules');
...
Use hooks.`,
    ...
    it('skips project rules when folder is untrusted', async () => {
      await createTestFile(
        path.join(projectRoot, TRAM_DIR, 'rules', 'r.md'),
        'Untrusted.',
      );
    ...
    it('loads global rules even when folder is untrusted', async () => {
      await createTestFile(
        path.join(homedir, TRAM_DIR, 'rules', 'g.md'),
        'Global.',
      );
      const result = await loadRules(projectRoot, false);
      expect(result.ruleCount).toBe(1);
    ...
    it('does not duplicate rules when projectRoot equals homedir', async () => {
      await createTestFile(
        path.join(homedir, TRAM_DIR, 'rules', 's.md'),
        'Shared.',
      );
    ...
    it('formats rules with source markers', async () => {
      await createTestFile(
        path.join(projectRoot, TRAM_DIR, 'rules', 'test.md'),
        'Content.',
      );
      const result = await loadRules(projectRoot, true);
      expect(result.content).toContain(
        `--- Rule from: ${TRAM_DIR}/rules/test.md ---`,
      );
    });
    ...
    it('reads global rules from QWEN_HOME when set', async () => {
      ...
      try {
        ...
        await createTestFile(
          path.join(homedir, TRAM_DIR, 'rules', 'fromLegacyHome.md'),
          'LegacyHome rule.',
        );
