...
import { TramIgnoreParser } from "./tramIgnoreParser.js";
import * as fs from "node:fs/promises";
import * as path from "node:path";
import * as os from "node:os";

describe("TramIgnoreParser", () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), "tramignore-test-"));
  });
  ...
  describe("when .tramignore exists", () => {
    beforeEach(async () => {
    ...
    it("should ignore files specified in .tramignore", () => {
      const parser = new TramIgnoreParser(projectRoot);
  ...
  describe("when .tramignore does not exist", () => {
