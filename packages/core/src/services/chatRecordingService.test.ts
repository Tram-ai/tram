...
describe("ChatRecordingService", () => {
  ...
  beforeEach(() => {
    ...
    // Mock jsonl-utils. writeLine is async â€?mockResolvedValue returns
  ...
  describe('flush', () => {
    it('resolves immediately on a service with no enqueued writes', async () => {
      ...
      // service should settle in a single microtask â€?important because
  ...
  describe('ensureChatsDir caching', () => {
    it('does not cache when mkdirSync throws so the next write retries', async () => {
      ...
      // â‰?rather than === leaves room for a future flush()-side retry.
  ...
  describe('recordAttributionSnapshot', () => {
    ...
    // the abandoned branch, so the dedup key has to clear â€?otherwise
    ...
    it('should re-write an identical snapshot after rewindRecording', async () => {
      ...
      // Same snapshot bytes â€?without the rewind reset this would dedup.
      chatRecordingService.recordAttributionSnapshot(baseSnapshot);
      await chatRecordingService.flush();
    ...
    // swallowed inside the service â€?otherwise it surfaces as an
