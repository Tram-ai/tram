...
describe("CronScheduler", () => {
  ...
  describe("create", () => {
    ...
    it("creates service action jobs with a readable summary", () => {
      const job = scheduler.create(
        "*/10 * * * *",
        { type: "service_restart", serviceName: "minecraft" },
        true,
      );
      ...
      expect(job.prompt).toBe("Restart service minecraft");
    });
  ...
  describe("getExitSummary", () => {
    ...
    it("returns summary with single job", () => {
      ...
      expect(summary).toContain("1 active scheduled job cancelled:");
    ...
    it("returns summary with multiple jobs", () => {
      ...
      const summary = scheduler.getExitSummary()!;
      expect(summary).toContain("2 active scheduled jobs cancelled:");
      expect(summary).toContain("check the build");
