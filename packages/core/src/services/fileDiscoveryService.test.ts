...
describe("FileDiscoveryService", () => {
  ...
  describe("initialization", () => {
    ...
    it("should load .tramignore patterns even when not in a git repo", async () => {
      await createTestFile(".tramignore", "secrets.txt");
      const service = new FileDiscoveryService(projectRoot);

      expect(service.shouldTramIgnoreFile("secrets.txt")).toBe(true);
      expect(service.shouldTramIgnoreFile("src/index.js")).toBe(false);
    });
  });
  ...
  describe("filterFiles", () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, ".git"));
      await createTestFile(".gitignore", "node_modules/\n.git/\ndist");
      await createTestFile(".tramignore", "logs/");
    });

    it("should filter out git-ignored and tram-ignored files by default", () => {
      const files = [
        "src/index.ts",
    ...
    it("should not filter files when respectGitIgnore is false", () => {
      ...
      const filtered = service.filterFiles(files, {
        ...
        respectTramIgnore: true, // still respect this one
      });
    ...
    it("should not filter files when respectTramIgnore is false", () => {
      const files = [
        "src/index.ts",
        "node_modules/package/index.js",
        "logs/latest.log",
  ...
  describe("shouldGitIgnoreFile & shouldTramIgnoreFile", () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, ".git"));
      await createTestFile(".gitignore", "node_modules/");
      await createTestFile(".tramignore", "*.log");
    });
    ...
    it("should return true for tram-ignored files", () => {
      const service = new FileDiscoveryService(projectRoot);

      expect(
        service.shouldTramIgnoreFile(path.join(projectRoot, "debug.log")),
      ).toBe(true);
    });

    it("should return false for non-tram-ignored files", () => {
      const service = new FileDiscoveryService(projectRoot);

      expect(
        service.shouldTramIgnoreFile(path.join(projectRoot, "src/index.ts")),
      ).toBe(false);
    });
  });
