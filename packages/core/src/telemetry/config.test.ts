...
describe("telemetry/config helpers", () => {
  ...
  describe("resolveTelemetrySettings", () => {
    ...
    it("uses env over settings and argv over env", async () => {
      ...
      const env = {
        TRAM_TELEMETRY_ENABLED: "1",
        TRAM_TELEMETRY_TARGET: "gcp",
        TRAM_TELEMETRY_OTLP_ENDPOINT: "http://env:4317",
        TRAM_TELEMETRY_OTLP_PROTOCOL: "http",
        TRAM_TELEMETRY_LOG_PROMPTS: "true",
        TRAM_TELEMETRY_INCLUDE_SENSITIVE_SPAN_ATTRIBUTES: "true",
        TRAM_TELEMETRY_OUTFILE: "env.log",
        TRAM_TELEMETRY_USE_COLLECTOR: "true",
    ...
    it("parses includeSensitiveSpanAttributes from settings and env", async () => {
      ...
      const resolvedEnvTrue = await resolveTelemetrySettings({
        env: {
          TRAM_TELEMETRY_INCLUDE_SENSITIVE_SPAN_ATTRIBUTES: "1",
        },
      ...
      const resolvedEnvFalse = await resolveTelemetrySettings({
        env: {
          TRAM_TELEMETRY_INCLUDE_SENSITIVE_SPAN_ATTRIBUTES: "false",
        },
        settings: { includeSensitiveSpanAttributes: true },
      });
    ...
    it("throws on unknown protocol values", async () => {
      const env = { TRAM_TELEMETRY_OTLP_PROTOCOL: "unknown" } as Record<
        string,
        string
      >;
    ...
    it("throws on unknown target values", async () => {
      const env = { TRAM_TELEMETRY_TARGET: "unknown" } as Record<
        string,
        string
      >;
      await expect(resolveTelemetrySettings({ env })).rejects.toThrow(
