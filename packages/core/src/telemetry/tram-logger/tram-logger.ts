...
export class TramLogger {
  private static instance: TramLogger;
  ...
  private constructor(config: Config) {
    this.config = config;
    this.debugLogger = createDebugLogger("TRAM_LOGGER");
  ...
  static getInstance(config?: Config): TramLogger | undefined {
    if (config === undefined || !config?.getUsageStatisticsEnabled())
      return undefined;
    if (!TramLogger.instance) {
      TramLogger.instance = new TramLogger(config);
    }

    return TramLogger.instance;
  }
    ...
    try {
      ...
      if (wasAtCapacity) {
        this.debugLogger.debug(
          `TramLogger: Dropped old event to prevent memory leak (queue size: ${this.events.size})`,
        );
      }
    } catch (error) {
      this.debugLogger.error("TramLogger: Failed to enqueue log event.", error);
    }
  }
  ...
  async flushToRum(): Promise<LogResponse> {
    if (this.isFlushInProgress) {
      this.debugLogger.debug(
        "TramLogger: Flush already in progress, marking pending flush.",
      );
  ...
  logApiResponseEvent(event: ApiResponseEvent): void {
    const rumEvent = this.createResourceEvent("api", "api_response", {
      ...
      snapshots: JSON.stringify({
        ...
        // TODO(merge-v0.15.10): tool_token_count removed from ApiResponseEvent
        // upstream; restore once tool-only tokens are tracked again.
      }),
    });
  ...
  private requeueFailedEvents(eventsToSend: RumEvent[]): void {
    ...
    this.debugLogger.debug(
      `TramLogger: Re-queued ${numEventsToRequeue} events for retry (queue size: ${this.events.size})`,
    );
  }
}
