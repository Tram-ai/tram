...
import { TramLogger, TEST_ONLY } from './tram-logger.js';
...
const makeFakeConfig = (overrides: Partial<Config> = {}): Config => {
  const defaults = {
    ...
    getAuthType: () => AuthType.TRAM_OAUTH,
    getMcpServers: () => ({}),
    getModel: () => 'test-model',
...
describe('TramLogger', () => {
  ...
  beforeEach(() => {
    ...
    (TramLogger as any).instance = undefined;
  });
  ...
  describe('getInstance', () => {
    it('returns undefined when usage statistics are disabled', () => {
      ...
      const logger = TramLogger.getInstance(config);
      expect(logger).toBeUndefined();
    });

    it('returns an instance when usage statistics are enabled', () => {
      const logger = TramLogger.getInstance(mockConfig);
      expect(logger).toBeInstanceOf(TramLogger);
    });

    it('is a singleton', () => {
      const logger1 = TramLogger.getInstance(mockConfig);
      const logger2 = TramLogger.getInstance(mockConfig);
