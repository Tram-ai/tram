...
describe("Server Config (config.ts)", () => {
  ...
  describe("model switching optimization (TRAM_OAUTH)", () => {
    it("should switch tram-oauth model in-place without refreshing auth when safe", async () => {
      const config = new Config(baseParams);
      ...
      // Establish initial tram-oauth content generator config/content generator.
  ...
  describe("Usage Statistics", () => {
    ...
    it("logs the session start event", async () => {
      ...
      expect(TramLogger.prototype.logStartSessionEvent).toHaveBeenCalledOnce();
    });
  });
...
describe("Model Switching and Config Updates", () => {
  ...
  it("should update contextWindowSize when switching models with hot-update", async () => {
    ...
    ).handleModelChange(AuthType.TRAM_OAUTH, false);

    // Verify all fields are updated
    const updatedConfig = config.getContentGeneratorConfig();
    expect(updatedConfig["model"]).toBe("tram-max");
  ...
  it("should trigger full refresh when switching to non-tram-oauth provider", async () => {
    const config = new Config(baseParams);

    // Initialize with tram-oauth
    const initialConfig: ContentGeneratorConfig = {
      ["model"]: "qwen3-coder-plus",
      ["authType"]: AuthType.TRAM_OAUTH,
      ["apiKey"]: "test-key",
