...
const MEMORY_SECTION_HEADER = "## TRAM Added Memories";
...
describe("MemoryTool", () => {
  ...
  describe("performAddMemoryEntry (static method)", () => {
    ...
    beforeEach(() => {
      testFilePath = path.join(os.homedir(), ".tram", DEFAULT_CONTEXT_FILENAME);
    });
  ...
  describe("getDefaultPermission and getConfirmationDetails", () => {
    ...
    it("should return confirmation details for global scope", async () => {
      ...
      if (result.type === "edit") {
        const expectedPath = path.join("~", ".tram", "QWEN.md");
        expect(result.title).toBe(
          `Confirm Memory Save: ${expectedPath} (global)`,
        );
        expect(result.fileName).toContain(path.join("mock", "home", ".tram"));
        expect(result.fileName).toContain("TRAM.md");
        expect(result.fileDiff).toContain("Index: TRAM.md");
        expect(result.fileDiff).toContain("+## TRAM Added Memories");
        expect(result.fileDiff).toContain("+- Test fact");
        expect(result.originalContent).toBe("");
        expect(result.newContent).toContain("## TRAM Added Memories");
    ...
    it("should prompt for scope selection when scope is not specified", async () => {
      ...
      if (result.type === "edit") {
        ...
        expect(result.fileName).toBe("TRAM.md");
        expect(result.fileDiff).toContain("Test fact");
        expect(result.fileDiff).toContain("--- TRAM.md");
        expect(result.fileDiff).toContain("+++ TRAM.md");
        expect(result.fileDiff).toContain("+- Test fact");
    ...
    it("should show correct file paths in scope selection prompt", async () => {
      ...
      if (result.type === "edit") {
        const globalPath = path.join("~", ".tram", "QWEN.md");
  ...
  describe("getDescription", () => {
    ...
    it("should return correct description for global scope", () => {
      ...
      const expectedPath = path.join("~", ".tram", "TRAM.md");
    ...
    it("should return correct description for project scope", () => {
      ...
      const expectedPath = path.join(process.cwd(), "TRAM.md");
    ...
    it("should show choice prompt when scope is not specified", () => {
      ...
      const globalPath = path.join("~", ".tram", "TRAM.md");
      const projectPath = path.join(process.cwd(), "TRAM.md");
