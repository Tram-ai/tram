...
> {
  ...
  private async extractModMetadata(
    ...
    if (fileBuffer && /\.(jar|zip)$/i.test(fileName)) {
      ...
      if (archiveResult && (archiveResult.version !== 'unknown' || archiveResult.loader !== 'unknown')) {
   ...
   */
  private readZipEntries(zipBuffer: Buffer, targetNames: string[]): Map<string, Buffer> {
    ...
    try {
      ...
      while (pos < cdOffset + cdSize && pos + 46 <= zipBuffer.length) {
        ...
        const entryName = zipBuffer.subarray(pos + 46, pos + 46 + fileNameLen).toString('utf-8');
  ...
  private async extractMetadataFromArchive(
    ...
    if (modToml) {
      try {
        ...
        let loader = 'forge';
        if (modLoader.toLowerCase().includes('neoforge') || modLoader.toLowerCase().includes('lowcodefml')) {
          loader = 'neoforge';
        }
        if (mods && mods.length > 0) {
          const mod = mods[0]!;
          return {
            name: (mod['displayName'] as string) || (mod['modId'] as string) || 'unknown',
    ...
    if (spongePlugins) {
      try {
        ...
        if (plugins && plugins.length > 0) {
          ...
          return {
            name: (plugin['name'] as string) || (plugin['id'] as string) || 'unknown',
    ...
    if (manifest) {
      try {
        ...
        const name = getAttr('Implementation-Title') || getAttr('Bundle-Name') || getAttr('Specification-Title');
        const version = getAttr('Implementation-Version') || getAttr('Bundle-Version') || getAttr('Specification-Version');
        if (name || version) {
          return {
            name: name || 'unknown',
  ...
  private murmur2(data: Uint8Array, seed: number = 1): number {
    ...
    switch (len) {
      case 3: h ^= (data[i + 2]! << 16) >>> 0; // falls through
      case 2: h ^= (data[i + 1]! << 8) >>> 0;  // falls through
      case 1: h ^= data[i]! >>> 0; h = Math.imul(h, m) >>> 0;
    }
  ...
  private async lookupCurseForgeByFingerprint(
    fingerprint: number,
  ): Promise<{ name: string; projectId: number; fileId: number; fileName: string; projectUrl: string; downloadUrl: string } | null> {
    try {
      const response = await fetch('https://curseforgeapi.912778.xyz/v1/fingerprints', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        debugLogger.warn(`CurseForge fingerprint API error: ${response.status}`);
        return null;
      }
  ...
  private async lookupHangarByHash(
    sha256Hash: string,
  ): Promise<{ name: string; projectUrl: string; versionName: string; versionVerified: boolean; channel?: string; platform: string } | null> {
    try {
      ...
      const semverLike = /^\d+\.\d+(\.\d+)?(-[\w.]+)?(\+[\w.]+)?$/.test(rawVersionName);
      const versionVerified = semverLike && channelName?.toLowerCase() === 'release';
  ...
  private async resolveHangarRealVersion(
    ...
    try {
      const url = new URL(`https://hangar.papermc.io/api/v1/projects/${owner}/${slug}/versions`);
      ...
      const releaseVersion = data.result.find(
        (v) => semverPattern.test(v.name) && v.channel?.name?.toLowerCase() === 'release',
      );
  ...
  private formatModrinthResult(modData: Record<string, unknown>): {
    ...
    const versions = (modData['game_versions'] as string[] | null | undefined) || [];
    ...
    const matchedFileName = primaryFile ? (primaryFile['filename'] as string) || '' : '';
  ...
  async execute(_signal: AbortSignal): Promise<ToolResult> {
    try {
      ...
      const requestedHash = hashType === 'sha1' ? sha1Hash : createHash(hashType).update(fileBuffer).digest('hex');
      const fileName = filePath.split(/[\\/]/).pop() || 'unknown';

      // Compute CurseForge fingerprint (MurmurHash2)
      const curseForgeFingerprint = this.computeCurseForgeFingerprint(fileBuffer);
      ...
      const [modrinthMatchData, curseForgeMatch, hangarMatch] = await Promise.all([
      ...
      if (!finalModrinthData) {
        const sha512Hash = createHash('sha512').update(fileBuffer).digest('hex');
        finalModrinthData = await this.lookupModrinthByHash(sha512Hash, 'sha512');
      }
      ...
      if (enrichedModInfo) {
        if (modrinthMatch) {
          ...
          if (enrichedModInfo.version === 'unknown' && modrinthMatch.versionNumber) {
            enrichedModInfo.version = modrinthMatch.versionNumber;
          }
          if (enrichedModInfo.loader === 'unknown' && modrinthMatch.compatible_loaders.length > 0) {
      ...
      if (!foundAny) {
        ...
        if (enrichedModInfo) {
          displayLines.push(`Try searching for "${enrichedModInfo.name}" manually.`);
        }
      }
