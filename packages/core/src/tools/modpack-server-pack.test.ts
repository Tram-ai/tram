...
describe('ModpackServerPackTool', () => {
  ...
  it('should validate action enum values', () => {
    ...
    expect((properties['action']['enum'] as string[])).toContain('curseforge-server-pack');
    expect((properties['action']['enum'] as string[])).toContain('modrinth-server-pack');
  });

  it('should accept curseforge-server-pack action', () => {
    const invocation = tool.build({ action: 'curseforge-server-pack', projectId: 12345 });
    expect(invocation).toBeDefined();
  });

  it('should accept modrinth-server-pack action with mrpackPath', () => {
    const invocation = tool.build({ action: 'modrinth-server-pack', mrpackPath: '/tmp/test.mrpack' });
    expect(invocation).toBeDefined();
  });
  ...
  describe('curseforge-server-pack', () => {
    ...
    it('should handle CurseForge API response with serverPackFileId', async () => {
      ...
      fetchSpy.mockResolvedValueOnce(
        new Response(JSON.stringify({ data: { name: 'Test Modpack', id: 12345 } }), { status: 200 }),
      );
      fetchSpy.mockResolvedValueOnce(
        new Response(JSON.stringify({
          data: [{ id: 100, displayName: 'TestFile', fileName: 'test.zip', downloadUrl: null, serverPackFileId: 200 }],
        }), { status: 200 }),
      );
      fetchSpy.mockResolvedValueOnce(
        new Response(JSON.stringify({
      ...
      fetchSpy.mockResolvedValueOnce(
        new Response(JSON.stringify({
          data: { id: 200, displayName: 'ServerPack', fileName: 'server.zip', downloadUrl: 'https://example.com/server.zip', serverPackFileId: null },
        }), { status: 200 }),
      );
      ...
      try {
        ...
        expect(parsed.serverPackDownloadUrl).toBe('https://example.com/server.zip');
      } finally {
    ...
    it('should handle CurseForge modpack without serverPackFileId', async () => {
      ...
      fetchSpy.mockResolvedValueOnce(
        new Response(JSON.stringify({ data: { name: 'ClientOnly', id: 99999 } }), { status: 200 }),
      );
      fetchSpy.mockResolvedValueOnce(
        new Response(JSON.stringify({
          data: [{ id: 300, displayName: 'Client', fileName: 'client.zip', downloadUrl: null, serverPackFileId: null }],
        }), { status: 200 }),
      );
      fetchSpy.mockResolvedValueOnce(
        new Response(JSON.stringify({
      ...
      try {
        const invocation = tool.build({ action: 'curseforge-server-pack', projectId: 99999 });
        const result = await invocation.execute(new AbortController().signal);
        const parsed = JSON.parse(result.llmContent as string);
  ...
  describe('modrinth-server-pack', () => {
    ...
    it('should parse a valid .mrpack and produce server assembly plan', async () => {
        ...
        files: [
          {
            ...
            hashes: { sha1: 'abc123' },
            downloads: ['https://cdn.modrinth.com/data/xxxx/versions/server-mod.jar'],
            env: { client: 'required', server: 'required' },
          },
          {
            path: 'mods/client-only-mod.jar',
            hashes: { sha1: 'def456' },
            downloads: ['https://cdn.modrinth.com/data/xxxx/versions/client-only-mod.jar'],
            env: { client: 'required', server: 'unsupported' },
          },
          {
            path: 'mods/both-mod.jar',
            hashes: { sha1: 'ghi789' },
            downloads: ['https://cdn.modrinth.com/data/xxxx/versions/both-mod.jar'],
          },
          {
            path: 'config/mod-config.toml',
            hashes: { sha1: 'jkl012' },
            downloads: ['https://cdn.modrinth.com/data/xxxx/versions/mod-config.toml'],
          },
          {
            path: 'resourcepacks/textures.zip',
            hashes: { sha1: 'mno345' },
            downloads: ['https://cdn.modrinth.com/data/xxxx/versions/textures.zip'],
          },
        ],
      };
      ...
      try {
        const invocation = tool.build({ action: 'modrinth-server-pack', mrpackPath });
        const result = await invocation.execute(new AbortController().signal);
        const parsed = JSON.parse(result.llmContent as string);
        ...
        const serverModPaths = parsed.serverMods.map((m: Record<string, string>) => m['path']);
        ...
        const clientModPaths = parsed.clientOnlyMods.map((m: Record<string, string>) => m['path']);
        ...
        expect(parsed.serverInstallerInfo.installerUrl).toContain('fabricmc.net');
      } finally {
    ...
    it('should write override files when outputDir is specified', async () => {
      ...
      try {
        const invocation = tool.build({ action: 'modrinth-server-pack', mrpackPath, outputDir });
        const result = await invocation.execute(new AbortController().signal);
        const parsed = JSON.parse(result.llmContent as string);
        ...
        expect(parsed.serverInstallerInfo.installerUrl).toContain('minecraftforge.net');

        expect(fs.existsSync(path.join(outputDir, 'config', 'test.toml'))).toBe(true);
        expect(fs.readFileSync(path.join(outputDir, 'config', 'test.toml'), 'utf-8')).toBe('key = "value"');
      } finally {
        fs.rmSync(tmpDir, { recursive: true, force: true });
      }
    });
    ...
    it('should handle all loader types for installer info', async () => {
      const loaderTests = [
        { deps: { minecraft: '1.20.1', forge: '47.2.0' }, expectedLoader: 'forge', urlContains: 'minecraftforge.net' },
        { deps: { minecraft: '1.20.1', 'fabric-loader': '0.14.22' }, expectedLoader: 'fabric', urlContains: 'fabricmc.net' },
        { deps: { minecraft: '1.20.1', 'quilt-loader': '0.20.0' }, expectedLoader: 'quilt', urlContains: 'quiltmc.org' },
        { deps: { minecraft: '1.20.1', neoforge: '20.4.100' }, expectedLoader: 'neoforge', urlContains: 'neoforged.net' },
      ];

      for (const loaderTest of loaderTests) {
        const tmpDir = path.join(os.tmpdir(), `tram-test-loader-${Date.now()}-${loaderTest.expectedLoader}`);
        const mrpackPath = path.join(tmpDir, 'test.mrpack');
        fs.mkdirSync(tmpDir, { recursive: true });
        fs.writeFileSync(mrpackPath, 'dummy');
        ...
        try {
          ...
          expect(parsed.serverInstallerInfo).toBeDefined();
          expect(parsed.serverInstallerInfo.installerUrl).toContain(loaderTest.urlContains);
        } finally {
          fs.rmSync(tmpDir, { recursive: true, force: true });
        }
      }
