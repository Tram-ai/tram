  ...
  parametersJsonSchema: {
    ...
    properties: {
      action: {
        type: 'string',
        enum: ['search', 'detail'],
        description: 'Action to perform: "search" to search videos by keyword, "detail" to get video details by BV number.',
      },
      keyword: {
        type: 'string',
      ...
      bvid: {
        type: 'string',
        description: 'Bilibili video BV number, e.g. "BV1hNFdz4EZp" (required when action is "detail").',
      },
      page: {
        type: 'number',
...
> {
  ...
  private async searchVideos(): Promise<BilibiliSearchResult[]> {
    ...
    if (!response.ok) {
      throw new Error(`Bilibili search API error: ${response.status} ${response.statusText}`);
    }
  ...
  private async getVideoDetail(): Promise<BilibiliVideoDetail> {
    ...
    if (!response.ok) {
      throw new Error(`Bilibili view API error: ${response.status} ${response.statusText}`);
    }
...
> {
  ...
  override validateToolParamValues(params: BilibiliVideoInfoParams): string | null {
    if (params.action === 'search' && (!params.keyword || params.keyword.trim().length === 0)) {
      return 'keyword is required for search action.';
    }
    if (params.action === 'detail' && (!params.bvid || params.bvid.trim().length === 0)) {
      return 'bvid is required for detail action.';
    }
    if (params.pageSize !== undefined && (params.pageSize < 1 || params.pageSize > 50)) {
