...
/**
 * Per-process cache for `.Tramignore` discovery. The same directories show
 ...
 * `TramIgnore`: dir → string | null (cached `.Tramignore` path or null)
 *
 * **Known staleness window:** a `.Tramignore` created mid-session, or a
...
const TramIgnoreCache = new Map<string, string | null>();
...
> {
  ...
  private async performRipgrepSearch(options: {
    ...
    if (filteringOptions.respectTramIgnore) {
      // Load .tramignore from each workspace directory, not just the primary one
      ...
      for (const searchPath of paths) {
        ...
        let TramIgnorePath = TramIgnoreCache.get(dir);
        if (TramIgnorePath === undefined) {
          const candidate = path.join(dir, ".Tramignore");
          TramIgnorePath = fs.existsSync(candidate) ? candidate : null;
          TramIgnoreCache.set(dir, TramIgnorePath);
          trimCache(TramIgnoreCache);
        }
        if (TramIgnorePath && !seenIgnoreFiles.has(TramIgnorePath)) {
          rgArgs.push("--ignore-file", TramIgnorePath);
          seenIgnoreFiles.add(TramIgnorePath);
        }
      }
    }
