...
> {
  ...
  override async execute(): Promise<ToolResult> {
    ...
    const mediaType =
      this.params.mediaType === "auto" || !this.params.mediaType
  ...
  private buildFfmpegArgs(
    ...
    if (mediaType === "image") {
      ...
      const q =
        qualityMap.image[quality as keyof typeof qualityMap.image] || "5";
      args.push("-q:v", q);
      ...
      const crf =
        qualityMap.video[quality as keyof typeof qualityMap.video] || "28";
      args.push(
        "-c:v",
        "libx264",
        "-crf",
        ...
        "aac",
        "-b:a",
        qualityMap.audio[quality as keyof typeof qualityMap.audio] || "192k",
      );
      ...
      Kind.Execute,
      {
        ...
        properties: {
          inputPath: {
            type: "STRING",
            description:
              "Path to the media file to compress (absolute or relative to workspace)",
          },
          outputPath: {
            type: "STRING",
            description:
              "Optional output path. Defaults to {name}_compressed.{ext} in same directory.",
          },
          mediaType: {
            type: "STRING",
            description:
              "Type of media: auto, image, video, or audio. Default is auto (detected from extension).",
          },
          quality: {
            type: "STRING",
            description:
              "Compression quality: low (smallest), medium (balanced), high (best quality). Default is medium.",
          },
          maxWidth: {
            type: "INTEGER",
            description:
              "Maximum width in pixels (optional, for image/video resize).",
          },
          maxHeight: {
            type: "INTEGER",
            description:
              "Maximum height in pixels (optional, for image/video resize).",
          },
        },
        required: ["inputPath"],
      },
