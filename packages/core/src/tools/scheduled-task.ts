  ...
  parametersJsonSchema: {
    ...
    properties: {
      action: {
        ...
        description:
          'Action: "create" a new scheduled task, "list" all tasks, "cancel" a task by ID, or "cancel_all" to stop all tasks.',
      },
      name: {
        type: "string",
      ...
      delaySeconds: {
        type: "number",
        description:
          "Seconds to wait before first execution. Default is 0 (immediate).",
      },
      intervalSeconds: {
        type: "number",
        description:
          "Repeat interval in seconds. 0 means one-shot (no repeat). Default is 0.",
      },
      maxRepeat: {
        type: "number",
        description:
          "Maximum repetitions. 0 means unlimited (until cancelled). Default is 0.",
      },
      taskId: {
        type: "string",
...
> {
  ...
  private listTasks(): ToolResult {
    ...
    const displayLines = tasks.map((t, idx) => {
      const repeatInfo =
        t.intervalSeconds > 0 ? `every ${t.intervalSeconds}s` : "one-shot";
...
> {
  ...
  override validateToolParamValues(params: ScheduledTaskParams): string | null {
    ...
    if (
      ...
      (!params.taskId || params.taskId.trim().length === 0)
    ) {
