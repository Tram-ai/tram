...
const schema: FunctionDeclaration = {
  ...
  parametersJsonSchema: {
    ...
    properties: {
      ...
      input: { type: 'string' },
      pattern: { type: 'string', description: 'Regex pattern for suppress/unsuppress rules' },
      ruleDescription: { type: 'string', description: 'Description for suppress rule' },
...
> {
  ...
  getDescription(): string {
    return tt('Manage service action: {{action}}', { action: this.params.action });
  }
  ...
  async execute(_signal: AbortSignal): Promise<ToolResult> {
    ...
    try {
      switch (action) {
        case 'list': {
          ...
          if (services.length === 0) {
            ...
            output = services.map(s => {
              const status = s.hasPendingAlert ? 'ALERT' : s.running ? 'RUNNING' : 'STOPPED';
        ...
        case 'register':
          if (!name || !this.params.command) throw new Error(tt('name and command required'));
        ...
        case 'log': {
          ...
          // Use log variable system — store full logs in file, return reference for LM
        ...
        case 'alert': {
          ...
          if (!snapshot.hasAlert) {
            ...
            output = tt('Alert detected for {{name}}. Please use analyze action to read alert logs.', { name });
          }
          break;
        }
        ...
        case 'analyze': {
          ...
          if (!snapshot.hasAlert) {
            ...
            // Use log variable system — store alert logs in file, return reference for LM
            manager.clearAlert(name);
            output = await manager.getAlertLogsAsVariable(name, this.params.mode || 'all');
          }
          break;
        }

        case 'send':
          if (!name || !this.params.input) throw new Error(tt('name and input required'));
          await manager.sendInput(name, this.params.input);
          output = tt('Sent input to {{name}}. Consider checking logs after a brief moment.', { name });
          break;

        case 'suppress': {
          if (!name || !this.params.pattern) throw new Error(tt('name and pattern required'));
          await manager.addLogPatternRule(name, {
            pattern: this.params.pattern,
            action: 'suppress',
            description: this.params.ruleDescription || `Suppress lines matching: ${this.params.pattern}`,
          });
          output = tt('Added suppress rule for {{name}}: {{pattern}}', { name, pattern: this.params.pattern });
          break;
        }

        case 'unsuppress': {
          if (!name || !this.params.ruleId) throw new Error(tt('name and ruleId required'));
          const removed = await manager.removeLogPatternRule(name, this.params.ruleId);
          output = removed
            ? tt('Removed suppress rule {{ruleId}} for {{name}}.', { ruleId: this.params.ruleId, name })
            : tt('Rule {{ruleId}} not found for {{name}}.', { ruleId: this.params.ruleId, name });
          break;
        }
        ...
        case 'list-rules': {
          ...
          if (rules.length === 0) {
            ...
            output = tt('Log pattern rules for {{name}}:', { name }) + '\n' +
              rules.map(r => `  [${r.id}] ${r.action}: /${r.pattern}/ - ${r.description} (applied ${r.appliedCount ?? 0} times)`).join('\n');
          }
          break;
        }
      ...
      output = tt('Error executing action {{action}}: {{error}}', { action, error: e instanceof Error ? e.message : String(e) });
    }
