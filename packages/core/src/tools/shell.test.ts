...
describe("ShellTool", () => {
  ...
  beforeEach(() => {
    ...
    mockConfig = {
      ...
      storage: {
        getUserSkillsDirs: vi.fn().mockReturnValue(["/test/dir/.tram/skills"]),
        getProjectTempDir: vi.fn().mockReturnValue("/tmp/tram-temp"),
  ...
  describe("build", () => {
    ...
    it("should throw an error for a directory within the user skills directory", async () => {
      expect(() =>
        shellTool.build({
          command: "ls",
          directory: "/test/dir/.tram/skills/my-skill",
          is_background: false,
        }),
      ).toThrow(
    ...
    it("should throw an error for the user skills directory itself", async () => {
      expect(() =>
        shellTool.build({
          command: "ls",
          directory: "/test/dir/.tram/skills",
          is_background: false,
        }),
      ).toThrow(
        "Explicitly running shell commands from within the user skills directory is not allowed. Please use absolute paths for command parameter instead.",
    ...
    it("should resolve directory path before checking user skills directory", async () => {
      expect(() =>
        shellTool.build({
          command: "ls",
          directory: "/test/dir/.tram/skills/../skills/my-skill",
          is_background: false,
        }),
      ).toThrow(
        "Explicitly running shell commands from within the user skills directory is not allowed. Please use absolute paths for command parameter instead.",
  ...
  describe("execute", () => {
    ...
    it('does not forward the turn signal into the background shell', async () => {
      ...
      turnAc.abort();
      // The signal handed to ShellExecutionService stays un-aborted �?      // the turn's abort doesn't propagate into the background shell.
    ...
    describe('long-running foreground hint', () => {
      ...
      it('honors the MIN_LONG_RUN_THRESHOLD_MS floor for pathological tiny timeouts', async () => {
        ...
        // since we're isolating the threshold logic from the abort path �?        // a regression that strips the `Math.max(...)` guard would fire
      ...
      it('never appends the long-run hint on background commands', async () => {
        ...
        expect(result.llmContent).not.toContain('foreground command ran for');
        // The hint text contains the literal `is_background: true` �?        // the background path's own llmContent doesn't, so this guards
    ...
    describe("addCoAuthorToGitCommit", () => {
      it("should add co-author to git commit with double quotes", async () => {
        ...
        expect(mockShellExecutionService).toHaveBeenCalledWith(
          expect.stringContaining(
            "Co-authored-by: Tram <tramr@alibabacloud.com>",
          ),
          expect.any(String),
          expect.any(Function),
          expect.any(AbortSignal),
      ...
      it('should not add co-author when only pr is enabled (commit off)', async () => {
        ...
        (mockConfig.getGitCoAuthor as Mock).mockReturnValue({
          ...
          pr: true,
          name: "Tram-Coder",
          email: "tram-coder@alibabacloud.com",
        });
      ...
      it("should add Co-authored-by trailer to the LAST -m when multiple are present", async () => {
        ...
        expect(observed).toMatch(
          /-m\s+"Body line 1\s+Co-authored-by: Tram-Coder <tram-coder@alibabacloud\.com>"/s,
        );
 ...
 spanning multiple lines"`;
      ...
      it('should add co-author to git commit -m"msg" shorthand (no space)', async () => {
        ...
        expect(mockShellExecutionService).toHaveBeenCalledWith(
          expect.stringContaining(
            'Co-authored-by: Tram-Coder <tram-coder@alibabacloud.com>',
          ),
          expect.any(String),
          expect.any(Function),
          expect.any(AbortSignal),
    ...
    describe('addAttributionToPR', () => {
      ...
      // warning to surface the skip when TRAM_DEBUG_LOG_FILE is set.
      it.each([
      ...
      // `curl -b "session=abc" && gh pr create --body "summary"` �?      // without segment scoping the body regex would match curl's
