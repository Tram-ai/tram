...
export class TramSessionManager {
  ...
  async loadSession(
    ...
    try {
      ...
      if (!fs.existsSync(filePath)) {
        console.log(`[TramSessionManager] Session file not found: ${filePath}`);
        return null;
      }
   ...
   */
  async listSessions(workingDir: string): Promise<TramSession[]> {
    try {
      const sessionDir = this.getSessionDir(workingDir);
      ...
      for (const file of files) {
        try {
          ...
          console.error(
            `[TramSessionManager] Failed to read session file ${file}:`,
            error,
          );
        }
      }
  ...
  async deleteSession(sessionId: string, workingDir: string): Promise<boolean> {
    try {
      ...
      if (fs.existsSync(filePath)) {
        ...
        console.log(`[TramSessionManager] Session deleted: ${filePath}`);
        return true;
      }
