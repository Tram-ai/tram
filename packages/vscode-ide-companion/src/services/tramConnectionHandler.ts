...
/**
 * Tram Connection Handler class
 * Handles connection, authentication, and session initialization
 */
export class TramConnectionHandler {
  /**
   * Connect to Tram service and establish session
   *
  ...
  async connect(
    ...
    console.log(`[TramAgentManager] 🚀 CONNECT() CALLED - ID: ${connectId}`);
    ...
    if (!sessionRestored) {
      ...
      try {
        ...
        if (needsAuth) {
          ...
          console.log(`[TramAgentManager] Error details:`, sessionError);
          throw sessionError;
        }
      }
    } else {
    ...
    console.log(`\n========================================`);
    console.log(`[TramAgentManager] ✅ CONNECT() COMPLETED SUCCESSFULLY`);
    console.log(`========================================\n`);
    return {
      sessionCreated,
      requiresAuth,
  ...
  private async newSessionWithRetry(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        console.error(
          `[TramAgentManager] Session creation attempt ${attempt} failed:`,
          errorMessage,
        );

        // If Tram reports that authentication is required, try to
        ...
        console.log(`[TramAgentManager] Retrying in ${delay}ms...`);
        await new Promise((resolve) => setTimeout(resolve, delay));
      }
    }
