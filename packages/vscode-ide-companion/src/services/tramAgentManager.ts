...
export class TramAgentManager {
  ...
  private sessionReader: TramSessionReader;
  private sessionManager: TramSessionManager;
  private connectionHandler: TramConnectionHandler;
  private sessionUpdateHandler: TramSessionUpdateHandler;
  ...
  constructor() {
    ...
    this.sessionReader = new TramSessionReader();
    this.sessionManager = new TramSessionManager();
    this.connectionHandler = new TramConnectionHandler();
    this.sessionUpdateHandler = new TramSessionUpdateHandler({});
  ...
  async getSessionList(): Promise<Array<Record<string, unknown>>> {
    ...
    try {
      ...
      const result = sessions.map(
        (session: TramSession): Record<string, unknown> => ({
  ...
  async createNewSession(
    ...
    this.sessionCreateInFlight = (async () => {
      try {
        ...
        // Try to create a new ACP session. If Tram asks for auth, let it handle authentication.
        try {
