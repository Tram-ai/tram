...
describe("imageSupport browser bundling", () => {
  it("does not leave tram-core runtime imports in the webview bundle", async () => {
    const result = await esbuild.build({
      ...
      write: false,
      logLevel: "silent",
      external: ["@tram-ai/tram-core"],
    });

    const output = result.outputFiles[0]?.text ?? "";

    expect(output).not.toContain("@tram-ai/tram-core");
    expect(output).not.toContain("supportedImageFormats.js");
  });

  it("does not leave tram-core runtime imports in the App webview bundle", async () => {
    const result = await esbuild.build({
      entryPoints: [
