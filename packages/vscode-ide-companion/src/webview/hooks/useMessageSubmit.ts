...
import { ZERO_WIDTH_SPACE, stripZeroWidthSpaces } from "@tram-ai/webui";
...
export const useMessageSubmit = ({
  ...
  const handleSubmit = useCallback(
    (e: React.FormEvent | React.KeyboardEvent, explicitText?: string) => {
      ...
      // Handle /login (and its alias /auth) — trigger interactive
      // login flow directly in the extension instead of sending the command
      // to the agent.
      const trimmedInput = textToSend.trim();
      if (trimmedInput === "/login" || trimmedInput === "/auth") {
        setInputText("");
        if (inputFieldRef.current) {
