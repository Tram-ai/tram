...
export class AuthMessageHandler extends BaseMessageHandler {
  ...
  canHandle(messageType: string): boolean {
    return ["auth", "login", "getAccountInfo"].includes(messageType);
  }
  ...
  /**
   * Set auth interactive handler â€?interactive auth flow.
   */
  ...
  /**
   * Handle auth â€?full interactive auth flow.
   *
        ...
        [
          {
            ...
            description:
              "Paid Â· Up to 6,000 requests/5 hrs Â· All Coding Plan Models",
  ...
  private async authCodingPlan(): Promise<void> {
    const region = await this.pick(
      [
        {
          label: 'ä¸­å›½ (China)',
          description: 'é˜¿é‡Œäº‘ç™¾ç‚?â€?aliyun.com',
          value: 'china' as const,
        },
        {
          label: 'Global',
          description: 'Alibaba Cloud â€?alibabacloud.com',
          value: 'global' as const,
        },
      ],
      "TRAM: Coding Plan Region",
      'Select region',
    );
    if (!region) {
      return;
