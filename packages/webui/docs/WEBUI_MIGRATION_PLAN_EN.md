...
1. Extract components from `vscode-ide-companion/src/webview/` into an independent `@tram-ai/webui` package
...
```typescript
// @tram-ai/webui/src/context/PlatformContext.ts
...
```javascript
// @tram-ai/webui/tailwind.preset.js
module.exports = {
  theme: {
    extend: {
      colors: {
...
```
Developer ──► @tram-ai/webui ──► vscode-ide-companion
  │              │                      │
...
// After: Update import
import { FileIcon } from '@tram-ai/webui';
// or import { FileIcon } from '@tram-ai/webui/icons';
```
...
1. **Tailwind Class Name Tree Shaking**
   ...
   - Solution: Consumer's `content` config needs to include `node_modules/@tram-ai/webui`
...
│     └── Edit files in @tram-ai/webui/src/                     │
│                                                                  │
...
│     └── import { Component } from '@tram-ai/webui'            │
...
1. **Single Source of Truth**: All UI components live in `@tram-ai/webui`
