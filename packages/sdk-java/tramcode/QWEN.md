...
- Working Directory: /Users/weigeng/repos/tram/packages/sdk-java

## Project Details

- **Group ID**: com.alibaba
- **Artifact ID**: tramcode-sdk (as per pom.xml)
...
- **API Layer**: Provides the main entry points through `TramCodeCli` class with simple static methods for basic usage
...
- `TramCodeCli`: Main entry point with static methods for simple queries
...
The project includes basic unit tests using JUnit 5. The main test class `TramCodeCliTest` demonstrates how to use the SDK to make simple queries to the TRAM CLI.
...
- `pathToTramExecutable`: Path to the TRAM CLI executable
...
- **Session creation**: Use `TramCodeCli.newSession()` to create a new session with custom options
...
- **Thread Naming**: "tram_code_cli-pool-{number}"
